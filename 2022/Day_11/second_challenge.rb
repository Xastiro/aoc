...
common_divisor = monkeys.map { |monkey| monkey[:modulo_worry] }.inject(&:*)

10000.times do
  monkeys.each do |monkey|
    monkey[:items].each do |item|
      worry = monkey[:operation].(item) % common_divisor
      monkey[:count] += 1
